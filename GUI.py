...
def generate_table():
    ...
    if " " not in expression:
        if "," not in expression:
            ...
            a = tabulate(table_data, headers=list(variables) + [expression], tablefmt="simple_grid")
            ...
            output.insert("Tabla de Resultados:")
            output.insert(tabulate(table_data, headers=headers, tablefmt="double_grid"))  # Imprimir la tabla de resultados
            # Comparación de los resultados de las dos expresiones
            if all(x == y for x, y in table_data):
                output.insert("Las expresiones son equivalentes lógicas.")  # Si todos los pares de resultados son iguales, las expresiones son equivalentes lógicamente
            else:
                output.insert("Las expresiones no son equivalentes lógicas.")  # Si hay al menos un par de resultados diferentes, las expresiones no son equivalentes lógicamente
        else:
            output.insert("Entre las expresiones añada una coma(todo sin espacios).")
    else:
        output.insert("Ingrese los datos sin espacios.")
...
output = CTkTextbox(master=output_frame,width=1000,height=300, activate_scrollbars=True, font=("Consolas", 20, "bold"))
output.grid()
